...
function getUniversalMessageId(message) {
  ...
  for (var i = 0; i < rawContentLines.length; i++){
    ...
    if (line.substring(0, 10).toLowerCase() === HEADER_FIELD_TO_USE_AS_MESSAGE_ID) {
      ...
      // or the sender (eg outlook which behaves differently) has a newline following the Message-ID key and the actual value
      // is on the next line
      if (typeof messageIdComponents[1] === "undefined") {
